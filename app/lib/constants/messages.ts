export const ERROR = {
  POST: {
    NO_TARGET_NICKNAME: "중고거래 상대의 닉네임을 적어주세요.",
    NO_ETC_PLATFORM_NAME: "거래 사이트의 이름을 적어주세요.",
    SHORT_CONTENT: "내용은 최소 30자 이상 적어주세요.",
    NO_USER_NICKNAME: "거래 사이트에서 사용하는 본인 닉네임을 적어주세요.",
    NO_USER_PASSWORD: "비 로그인 사용자는 게시글 비밀번호를 적어주세요.",
    NO_RESULT_DB: "게시글 생성 오류. 잠시 후 다시 시도해주세요.",
    NO_AUTH: "로그인 후 시도해주세요.",
    NO_MATCH_AUTH: "사용자가 생성자와 일치하지 않습니다.",
  },
  COMMENT: {
    NO_AUTH: "로그인 후 시도해주세요.",
    NO_RESULT_DB: "댓글 생성 오류. 잠시 후 다시 시도해주세요.",
    NO_MATCH_AUTH: "사용자가 생성자와 일치하지 않습니다.",
  },
};
